...
# circularity  for decimal days < 0.0 or > 1.0
        day = day % 1.0
# hack rbh 18/2/20
# to support negative offsets in the timedelta parameter
        hours = 24.0 * day
        h = int(hours)
        minutes = (hours - h) * 60
        m = int(minutes)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 3:
#         2 parameters
        minutesDelay = int(sys.argv[2]) + 60 * ((24 + datetime.now().hour - datetime.utcnow().hour) % 24)
        tz = timezone(timedelta(minutes=minutesDelay))
        mytz = datetime.now(tz)
        s = Sun( mytz, 52.01355000660077, -2.5974807343283923)
        if sys.argv[1] == "sunrise":
            outputTime = s.sunrise()
        elif sys.argv[1] == "sunset":
            outputTime = s.sunset()
        else:
            print("incorrect parameter pattern")
            sys.exit(1)
        print(str(outputTime)[:5])
    else:
        print("incorrect parameter pattern")
